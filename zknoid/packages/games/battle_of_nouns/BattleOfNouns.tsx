...
import {
  useBattleOfNounsMatchQueueStore,
  useObserveBattleOfNounsMatchQueue,
...
import { battleOfNounsConfig } from "./config";
...
export default function BattleOfNouns({
  params,
}: {
  ...
  //   const battleOfNounsLogic = client_.runtime.resolve("BattleOfNounsLogic");

  //   const tx = await client.transaction(
  //     PublicKey.fromBase58(networkStore.address!),
  //     async () => {
  //       // await battleOfNounsLogic.hideNumber(Field.from(number));
  //     }
  //   );
  ...
  //       PublicKey.fromBase58(networkStore.address!),
  //       async () => {
  //         // await battleOfNounsLogic.guessNumber(Field.from(number));
  //       }
  //     );
  ...
  const collectPending = async () => {
    const battleOfNounsLogic = client.runtime.resolve("BattleOfNounsLogic");

    const tx = await client.transaction(
      sessionPrivateKey.toPublicKey(),
      async () => {
        battleOfNounsLogic.collectPendingBalance();
      }
    );
  ...
  return (
    <GamePage
      gameConfig={battleOfNounsConfig}
    ...
    >
      ...
      >
        ...
        <GameWidget
          author={battleOfNounsConfig.author}
          isPvp
        ...
        >
          {networkStore.address ? (
            <>
              {!competition ? (
                <GameWrap>
                  <UnsetCompetitionPopup gameId={battleOfNounsConfig.id} />
                </GameWrap>
              ) : (
                <>
                  {gameState == GameState.Won &&
                    ...
                    !rateGameStore.ratedGames.find(
                      (game) => game.gameId == battleOfNounsConfig.id
                    ) ? (
                      <GameWrap>
                        <RateGame
                          gameId={battleOfNounsConfig.id}
        ...
        <Competition
          ...
          competition={
            competition && {
              ...
              game: {
                id: battleOfNounsConfig.id,
                name: battleOfNounsConfig.name,
                rules: battleOfNounsConfig.rules,
                rating: 5,
                author: battleOfNounsConfig.author,
              },
