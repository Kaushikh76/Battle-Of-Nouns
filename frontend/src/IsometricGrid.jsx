...
const IsometricGrid = ({ username, tribe, characterIndex }) => {
  ...
  const tileImages = {
    ...
    tree: 'src/assets/Grass_tile_01.png',
    flower: 'src/assets/Flower_tile_01.png',
    house: 'src/assets/marine_house.png',
    sawmill: 'src/assets/Flower_tile_01.png',
    garden: 'src/assets/Flower_tile_01.png',
    captured_house: 'src/assets/Flower_tile_01.png',
  ...
  const initializeGrid = useCallback(() => {
    ...
    for (let i = 0; i < gridSize * gridSize / 5; i++) {
      ...
      newGrid[row][col] = ['tree', 'flower', 'house'][Math.floor(Math.random() * 3)];
    }
    
    const initialPosition = getRandomCornerPosition();
    newGrid[initialPosition.row][initialPosition.col] = 'base';
    ...
    const botInitialPosition = getRandomCornerPosition();
    newGrid[botInitialPosition.row][botInitialPosition.col] = 'base';
  ...
  const checkGameEnd = useCallback(() => {
    if (characters.length === 0) {
      setGameOver(true);
      setWinner(botTribe);
    } else if (botCharacters.length === 0) {
      setGameOver(true);
      setWinner(tribe);
    }
  }, [characters, botCharacters, botTribe, tribe]);

  const endTurn = useCallback(() => {
    setNouns(prevNouns => {
      let newNouns = prevNouns + 2;
      grid.forEach(row => row.forEach(tile => {
        if (tile === 'sawmill' || tile === 'garden') newNouns += 1;
        if (tile === 'captured_house') newNouns += 2;
      }));
      return newNouns;
    });
    ...
    setCharacters(prevCharacters => 
      prevCharacters.map(char => 
        char.id === selectedCharacterId 
          ? { ...char, position: { row: rowIndex, col: colIndex } } 
          : char
      )
    );
    setCanAct(false);
  ...
  const attack = useCallback((attacker, target) => {
      ...
      setCharacters(prevCharacters =>
        prevCharacters.map(char =>
          char.id === target.id
            ...
            : char
        ).filter(char => char.hp > 0)  // Remove characters with 0 hp
      );
    } else {
    ...
    checkGameEnd();
    endTurn();
  }, [characterStats, endTurn, tribe, checkGameEnd]);
  ...
  const performAction = useCallback((rowIndex, colIndex) => {
    ...
    setGrid(prevGrid => {
      ...
      switch (tileType) {
        case 'tree':
          if (nouns >= 3) {
            newGrid[rowIndex][colIndex] = 'sawmill';
            setNouns(prev => prev - 3);
          }
          break;
        case 'flower':
          if (nouns >= 3) {
            newGrid[rowIndex][colIndex] = 'garden';
            setNouns(prev => prev - 3);
          }
          break;
        case 'house':
          newGrid[rowIndex][colIndex] = 'captured_house';
          break;
      }
      return newGrid;
    });
    ...
    setCanAct(false);
    endTurn();
  }, [canAct, characters, grid, nouns, selectedCharacterId, endTurn]);
  ...
  }, [nouns, characters, tribe, getInitialHp, endTurn]);
  ...
  const handleBotAction = useCallback((action) => {
    ...
    switch (action.type) {
      case 'move':
        ...
        break;
      case 'harvest':
        performBotAction(action.to.row, action.to.col);
        break;
      case 'capture':
  ...
  const performBotAction = useCallback((row, col) => {
    setGrid(prevGrid => {
      const newGrid = [...prevGrid];
      const tileType = newGrid[row][col];
      switch (tileType) {
        case 'tree':
          if (botNouns >= 3) {
  ...
  const captureBotHouse = useCallback((row, col) => {
    setGrid(prevGrid => {
      const newGrid = [...prevGrid];
      if (newGrid[row][col] === 'house') {
        newGrid[row][col] = 'captured_house';
      }
      return newGrid;
    });
  }, []);
  ...
  const spawnBotCharacter = useCallback((row, col) => {
    ...
    setBotNouns(prev => prev - 3);
  }, [botNouns, botCharacters, botTribe, getInitialHp]);
  ...
  const handleTileClick = useCallback((rowIndex, colIndex) => {
    ...
    const activeCharacter = characters.find(char => char.id === selectedCharacterId);
    if (activeCharacter && isValidMove(rowIndex, colIndex, activeCharacter.position)) {
    ...
    } else if (activeCharacter && rowIndex === activeCharacter.position.row && colIndex === activeCharacter.position.col && canAct) {
      ...
      >
        {grid.map((row, rowIndex) => (
          row.map((tile, colIndex) => {
            const character = characters.find(char => char.position.row === rowIndex && char.position.col === colIndex);
            const botCharacter = botCharacters.find(char => char.position.row === rowIndex && char.position.col === colIndex);
            const isVisible = !fogGrid[rowIndex][colIndex];
            const isAvailableMove = selectedCharacterId !== null && isValidMove(rowIndex, colIndex, characters.find(char => char.id === selectedCharacterId)?.position);
                ...
                <div className="w-full h-full relative">
                  ...
                  {(character || botCharacter) && (
                    <div className="absolute w-full h-full">
                      <img 
                        src={characterImages[(character || botCharacter).tribe][(character || botCharacter).characterIndex]} 
                      ...
                      <div className="absolute bottom-0 left-0 right-0 h-2 bg-red-500">
                        <div 
                          ...
                          style={{ width: `${((character || botCharacter).hp / getInitialHp((character || botCharacter).characterIndex)) * 100}%` }}
                        ></div>
                      </div>
                    </div>
                  )}
  ...
  }, [grid, fogGrid, characters, botCharacters, tileSize, zoom, pan, isDragging, dragStart, selectedCharacterId, handleTileClick, isValidMove, isEnemy, getInitialHp]);
  ...
  const renderTileInfo = useCallback(() => {
    ...
    switch (type) {
      case 'house':
        return (
          <div className="space-y-2">
            <p className="font-bold">Uncaptured House</p>
            <p>Owner: None</p>
      ...
      case 'captured_house':
        return (
          <div className="space-y-2">
            <p className="font-bold">Captured House</p>
            <p>Owner: {username}</p>
            <p>Provides 2 nouns each turn.</p>
            {canAct && nouns >= 3 && (
              <button
      ...
      case 'flower':
        return (
          <div className="space-y-2">
            ...
            <p>{type === 'tree' ? 'Can be harvested for Saw mill.' : 'Can be harvested for flowers.'}</p>
            {canAct && nouns >= 3 && (
              <button
                onClick={() => performAction(row, col)}
  ...
  }, [selectedTile, characters, username, canAct, nouns, spawnCharacter, performAction]);
  ...
  }, [username, tribe, nouns, characterStats]);
      ...
      <div className="w-full h-full bg-beige rounded-lg overflow-hidden shadow-lg">
        {gameOver ? (
          <div className="p-4 text-center">
            ...
            {/* Add a restart button or other options here if desired */}
          </div>
        ) : (
